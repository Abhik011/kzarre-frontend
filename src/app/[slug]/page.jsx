...
export default function CmsPage({ params }) {
  ...
  const fetchPage = async () => {
    try {
      ...
      const res = await fetch(
        `${API}/api/admin/public/pages/${slug}`
      );
  ...
  const pageType = page.type || "generic"; // faq | policy | about | contact | generic
  ...
  const seoDescription =
    page.seoDescription ||
    `Read more about ${page.title} on our website.`;

  /* ================= FAQ JSON-LD (GOOGLE RICH RESULTS) ================= */

  const faqSections =
    pageType === "faq"
      ? (page.sections || []).filter((s) => s.type === "qa" && s.meta?.question)
      : [];
...
function FaqAccordion({ sections }) {
  ...
  return (
    <div className="faq-accordion">
      {faqItems.map((s, i) => {
        const question = s.meta?.question?.trim() || "Add a question";
