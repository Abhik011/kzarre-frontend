...
export default function CheckoutPage() {
  ...
  const [maxQty, setMaxQty] = useState(999);
...
useEffect(() => {
  async function loadProduct() {
    try {
      ...
      // ✅ SET MAX STOCK LIMIT FROM PRODUCT
      if (data.product?.stockQuantity !== undefined) {
        setMaxQty(data.product.stockQuantity);

        // ✅ AUTO FIX QTY IF URL QTY IS GREATER THAN STOCK
        setQty((prev) =>
          prev > data.product.stockQuantity
  ...
  if (qty > maxQty) {
    showMessage(`Only ${maxQty} units available for this product`);
    return;
  }

  const userId =
    typeof window !== "undefined"
      ? localStorage.getItem("kzarre_user_id")
      : null;
  const email =
    typeof window !== "undefined"
      ? localStorage.getItem("kzarre_remember_email")
      : null;
  ...
  try {
    ...
    window.location.href = `/payment?order=${encodeURIComponent(
      data.orderId
    )}`;
  } catch (err) {
